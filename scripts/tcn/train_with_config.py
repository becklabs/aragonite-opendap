...
from oadap.prediction.tcn.model import RegressionTCN, RegressionTCNv0, SpatialRegressionTCNv1
...
def prepare_datasets(X, y, config, device):
    ...
    assert X.shape[1] == y.shape[1], "X and y must have the same number of windows per location"
    nx, nw, nt, nf = X.shape
    _, _, nl = y.shape
    ...
    test_dataset = TempDataset(X[test_inds], y[test_inds], X_scaler=train_dataset.X_scaler, y_scaler=train_dataset.y_scaler, device=device)
    val_dataset = TempDataset(X_val, y_val, X_scaler=train_dataset.X_scaler, y_scaler=train_dataset.y_scaler, device=device)
...
def create_dataloaders(train_dataset, test_dataset, val_dataset, config):
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=config['training']['batch_size'])
...
def train(model, train_dataloader, test_dataloader, val_dataloader, config, device, start_epoch=0):
    optimizer = torch.optim.Adam(model.parameters(), lr=float(config['training']['learning_rate']))
    criterion = nn.MSELoss() if config['training']['loss_function'] == 'mse' else nn.L1Loss()

    if config['training']['resume_from_checkpoint']:
        checkpoint_path = os.path.join(config['training']['checkpoint_dir'], config['training']['resume_checkpoint'])
        model, optimizer, X_scaler, y_scaler, start_epoch = load_checkpoint(checkpoint_path, model, optimizer, device)
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        if epoch % config['wandb']['save_interval'] == 0:
            save_checkpoint(model, optimizer, epoch, train_dataloader.dataset.X_scaler, train_dataloader.dataset.y_scaler, config)

        logger.info(f"Epoch {epoch}: Train Loss: {avg_train_loss:.4f}, Test Loss: {avg_test_loss:.4f}, Val Loss: {avg_val_loss:.4f}")
...
def main():
    config = load_config('config/tcn/v0.yaml')
    device = set_device(config['device'])
    logger.info(f"Using device: {device}")

    wandb.init(project=config['wandb']['project'], config=config, resume=config['training']['resume_from_checkpoint'])

    X, y = load_data(config)
    train_dataset, test_dataset, val_dataset = prepare_datasets(X, y, config, device)
    train_dataloader, test_dataloader, val_dataloader = create_dataloaders(train_dataset, test_dataset, val_dataset, config)
